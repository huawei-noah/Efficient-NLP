...
from .dataset import glue_tasks_num_labels
from ..data.utils import AugmentedInputExample, InputFeaturesV2
...
def cached_model_outputs(
    ...
    **kwargs,
    ...
    num_augments = max(len(getattr(ex, "augmented_examples", [])) for ex in examples)
    dataset = _Dataset(
        examples,
        tokenizer,
        max_seq_length,
