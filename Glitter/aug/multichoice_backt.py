...
from glitter import HellaSwagProcessor, HellaSwagExample, copy_dev_file
...
logger = logging.getLogger("MultiChoice-bt")
...
def _to_json(
    question: str, id: str, src: Optional[str] = None
) -> Dict[str, Any]:
...
def prepare_outputs(
    ex, augmented_texts: List[str], lang: str, tokenize: bool
) -> List[Dict[str, Any]]:
    augmented_samples = []
    for j, aug_text in enumerate(augmented_texts):
        orig_aug = aug_text

        aug_text = f"{aug_text} {ex.ctx_b}"

        if tokenize:
            aug_text = _tokenize(aug_text)
            orig_aug = _tokenize(orig_aug)

        aug_json = _to_json(aug_text, f"aug-{j + 1}", src=f"en-{lang}")
        aug_json["ctx_a"] = orig_aug
        augmented_samples.append(aug_json)
    return augmented_samples
...
def main():
    ...
    with out_path.open("w", encoding="utf-8") as writer:
        ...
        for i in tqdm(range(num_batches), total=num_batches):
            ...
            src_batch = [en2x.encode(getattr(ex, field)) for ex in batch]
